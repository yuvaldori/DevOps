...
ant_params="-Dnew.build.number=$new_build_number -Dbuild.type=$build_type -Dgs.product.version=$gs_product_version -Dtgrid.suite.target-jvm=$tgrid_suite_target-jvm -DbuildDate-jvm=$buildDate -Dcvs.root=$cvs_root -Dgs.internal.systems.not.available=$gs_internal_systems_not_available -Dbuild.qa.suite.type=$build_qa_suite_type -Dtgrid.suite.factory-class=$tgrid_suite_factory-class -Dtgrid.suite.launcher-class=$tgrid_suite_launcher-class -Dpermutation.file.path=$permutation_file_path -Dbuild.qa.suite.version=$build_qa_suite_version -Dtgrid.suite.custom.includepackage=$tgrid_suite_custom_includepackage -Dtgrid.suite.custome.excludepackage=$tgrid_suite_custome_excludepackage -Dtgrid.suite.custom.jvmargs=$tgrid_suite_custom_jvmargs -Djvm.size.args=$jvm_size_args -Dmilestone=$milestone -Dbuild.gs.version=$build_gs_version -Dsvn.revision=$svn_revision -Drelease.build=$release_build -Dsvnant.repository.user=$svnant_repository_user -Dsvnant.repository.passwd=$svnant_repository_passwd -Dput.user=$put_user -Dpublish.package.to.S3=$publish_package_to_S3 -Ds3.publish.folder=$s3_publish_folder -Dpublish.jars.to.S3=$publish_jars_to_S3 -Dmaven.repo.local=$maven_repo_local -Dbuild.timestamp=$build_timestamp -Ds3.dev.and.packages.publish.folder=$s3_dev_and_packages_publish_folder"
if [[ $list ]]
then
    echo "yes" > send.email
    for line in "${list[@]}"
